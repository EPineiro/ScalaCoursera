...
class FunSetSuite extends FunSuite {
  ...
  test("intersection contains commons elements") {
    new TestSets {
      val u1 = union(s1, s2)
      val u2 = union(s2, s3)
  ...
  test("filter test") {
   ...
   test("forall test") {
    new TestSets {
      val u1 = union(s2, s4)
      val u2 = union(u1, s3)
      def empty_set: Set = x => false 
      
      assert(forall(u1, x => x % 2 == 0), "forall 1")
      assert(!forall(u2, x => x % 2 == 0), "forall 2")
      assert(forall(empty_set, x => x % 2 == 0), "forall empty set")
    }
  }
 ...
 test("exists test") {
    new TestSets {
      ...
      assert(!exists(u1, x => x % 3 == 0), "exists 1")
      assert(exists(u2, x => x % 3 == 0), "exists 2")
    }
  }
